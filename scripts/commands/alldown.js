...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (content.includes("https://fb.watch/") || content.includes("https://www.facebook.com")) {
    const fbApiResponse = await axios.get(`https://api.nayan-project.repl.co/nayan/alldownv3?key=Nayan_6933458141&url=${content}`);
    const fbVideoUrl = fbApiResponse.data.video;
  const cap = fbApiResponse.data.cap;
    ...
    api.sendMessage(
      ...
      event.threadID, event.massageID
    );
    ...
    const instagramApiResponse = await axios.get(`https://api.nayan-project.repl.co/nayan/alldownv3?key=Nayan_6933458141&url=${content}`);
    const instagramVideoUrl = instagramApiResponse.data.video;
    const capt = instagramApiResponse.data.cap;
    ...
    content.includes("https://youtu.be/")
  ) {
    // YouTube video download logic
    const youtubeApiResponse = await axios.get(`https://api.nayan-project.repl.co/nayan/alldownv4?key=Nayan_6933458141&url=${content}`);
    const youtubeVideoUrl = youtubeApiResponse.data.videos;
    const title = youtubeApiResponse.data.cap;
