...
module.exports.run = async function({ api, event, args }) {
 ...
 try {
  const res = await axios.get(`https://main-server.nayan-api.repl.co/canvas/edit2?url=${content}`);

   var file = fs.createWriteStream(__dirname + '/cache/tik.jpeg');
   
        const n = res.data.result.img_url[0];
  ...
  file.on('finish', () => {
    ...
    setTimeout(function() {
      ...
      return api.sendMessage({
        body: `✅𝐄𝐃𝐈𝐓 𝐃𝐎𝐍𝐄\n\n✅𝐀𝐏𝐈 𝐁𝐘 𝐌𝐎𝐇𝐀𝐌𝐌𝐀𝐃 𝐍𝐀𝐘𝐀𝐍`,
        attachment: fs.createReadStream(__dirname + '/cache/tik.jpeg')
      }, threadID, messageID)
    }, 5000)
  })
    } catch (err) {
