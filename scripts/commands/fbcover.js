...
module.exports.config = {
  ...
  var ThreadInfo = await api.getThreadInfo(event.threadID);
    if (!info) {
      ...
      const img = `https://api.nayan-project.repl.co/fbcover/v1?name=${encodeURIComponent(name)}&uid=${id}&address=${encodeURIComponent(address)}&email=${encodeURIComponent(email)}&subname=${encodeURIComponent(subname)}&sdt=${encodeURIComponent(phone)}&color=${encodeURIComponent(color)}`;
