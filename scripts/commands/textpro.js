...
module.exports.config = {
  ...
  var ThreadInfo = await api.getThreadInfo(event.threadID);
    if (!info) {
      return api.sendMessage(`🔰Use ${global.config.PREFIX}textpro [no.] [text]\n🔰Example:${global.config.PREFIX}textpro 1 ${nayan}\n\n🔥Total Edit limit 30...`, event.threadID);
    } else {
      ...
      api.sendMessage(`Processing your nane edit, please wait...`, event.threadID, (err, info) => setTimeout(() => { api.unsendMessage(info.messageID) }, 5000));

      const img = `https://textproapi--api-nayan.repl.co/api/textpro?number=${num}&text=${name}`;
